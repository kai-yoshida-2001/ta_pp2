...
guest_file = open('./input/???.txt', 'r', encoding='utf-8')
...
for guest in guest_file:
    ...
    # Q2. '敬具'を右寄せに指定しよう
    doc.add_paragraph('敬具').alignment = WD_ALIGN_PARAGRAPH.???
    # Q3. '記'を中央寄せに指定しよう
    doc.add_paragraph('記').alignment = WD_ALIGN_PARAGRAPH.???
    doc.add_paragraph('\t日時：4月1日　19:00')
    doc.add_paragraph('\t会場：秋田県立大学')
    # Q4. '以上'を右寄せに指定しよう
    doc.add_paragraph('以上').alignment = WD_ALIGN_PARAGRAPH.???
    doc.add_page_break()
