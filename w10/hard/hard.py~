...
org_ss = ezsheets.???(org_ss_id)

file_name = 'ta_pp2_normal.xlsx'
# Q2.'Excel'拡張子でダウンロードしよう
org_ss.downloadAs???(file_name)
# Q3. 自分のDriveに'upload'しよう
new_ss = ezsheets.???(file_name)
os.remove(file_name)
# Q4.'title'と'id'を取得しよう
print(f'{new_ss.???}を複製しました．\nID：{new_ss.???}')
...
# Q5.シートを新規作成しよう
result_s = new_ss.???(title='集計結果')
# Q6.行にデータを書き込もう
result_s.???(1, ['名簿', '点数', '偏差値'])
for i, (name, score, deviation_score) in enumerate(zip(names, scores, deviation_scores), start=2):
    # Q7.行にデータを書き込もう
    result_s.???(i, [name, score, round(deviation_score, 2)])

final_row = len(names) + 3
# Q8.行にデータを書き込もう
result_s.???(final_row, ['合計値', total])
result_s.???(final_row + 1, ['平均', round(average, 2)])
result_s.???(final_row + 2, ['標準偏差', round(std_dev, 2)])
